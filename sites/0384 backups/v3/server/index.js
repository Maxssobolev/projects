...
app.get('/api/homework', function (req, res) {
  pool.query(
    `SELECT subjects_homework.content as content, subjects_homework.title as title, DATE_FORMAT(subjects_homework.date, '%d.%m.%Y') as date, subjects.title as subjtitle FROM subjects_homework INNER JOIN subjects ON subjects.id = subjects_homework.subject_id ORDER BY date DESC`,
    (err, rows) => {
      if (err) {
        res.send(err);
      } else {
