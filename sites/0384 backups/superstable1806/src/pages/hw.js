          ...
          return (
            <div
              ...
              {...(deadline && moment().isAfter(deadline)
                ? { style: { opacity: '.65', order: `${100000 + index}`, position: 'relative' } }
                : { style: { order: `${100000 - index}`, position: 'relative' } })}>
              {this?.userData?.admin == 'yes' && (
                <Button
                  variant="danger"
                  onClick={() => this.handleDelete(res.id, 'subjects_homework', res.title)}
