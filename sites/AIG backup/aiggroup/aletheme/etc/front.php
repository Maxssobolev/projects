...
function ale_enqueue_scripts() {
	...
	wp_register_script( 'html5-shim', 'https://html5shim.googlecode.com/svn/trunk/html5.js', array( 'jquery' ), ALETHEME_THEME_VERSION, false );
	// add modernizr
	...
	if (!$thumbdone) {
		...
		if ($image) {
			...
			if (isset($matches[1])) {
				...
				if ($pos === false) {
					if (stristr($image, 'https://') || stristr($image, 'https://')) {
						$og_image = $image;
					} else {
