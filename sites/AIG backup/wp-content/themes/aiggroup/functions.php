...
function callBack(){
    ...
    $mail = new CURLmail('office@aiggroup.ru', "Заявка с сайта", $body_message);
    $res=$mail->send();
...
function callBack(){
	...
	$executor = ''; //обработать добавление исполнителя
...
function programmBuy(){
	...
	$mail = new CURLmail('r.miti@aiggroup.ru', "Покупка программы 1С", $body_message);
	$res=$mail->send();
    echo  $res;
